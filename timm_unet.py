...
class Unet(nn.Module):
    ...
    def __init__(
            self,
            # backbone='maxvit_rmlp_base_rw_384.sw_in12k_ft_in1k',
            # backbone='maxvit_rmlp_base_rw_384.sw_in12k_ft_in1k',
        ...
        if self.normalize_input:
            self.in_norm = torch.nn.InstanceNorm2d(in_chans, affine=False)
    ...
    def forward(self, x: torch.Tensor):
        if self.normalize_input:
            x = self.in_norm(x)  # Normalize phase contrast images to 0 mean and unit variance
        x = self.encoder(x)
...
class UnetDecoder(nn.Module):
    ...
    def __init__(
        ...
        #     self.final_conv.append(nn.Conv2d(out_channels[-1], final_channels, kernel_size=(1, 1)))
        self.heads = heads
        self.final_conv_dapi = nn.Sequential(*[
            Conv2dBnAct(out_channels[-1], out_channels[-1], kernel_size=(3, 3), padding=3//2),
            nn.Conv2d(out_channels[-1], final_channels, kernel_size=(1, 1), padding=1//2)
        ])
        self.final_conv_tdp43 = nn.Sequential(*[
            Conv2dBnAct(out_channels[-1], out_channels[-1], kernel_size=(3, 3), padding=3//2),
            nn.Conv2d(out_channels[-1], final_channels, kernel_size=(1, 1), padding=1//2)
        ])  
        self._init_weight()
    ...
    def forward(self, x: List[torch.Tensor]):
        ...
        for i, b in enumerate(self.blocks):
            ...
            x = b(x, skip)
        dapi = self.final_conv_dapi(x)
        tdp43 = self.final_conv_tdp43(x)
        return dapi, tdp43
